...
class _FullCalendarState extends State<FullCalendar> {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime lastDate =
        DateTime.parse("${partsEnd.first}-${(int.parse(partsEnd[1]) + 1).toString().padLeft(2, '0')}-01 23:00:00.000")
    ...
    if (firstDate.year == lastDate.year && firstDate.month == lastDate.month) {
      ...
      final _index = months.indexWhere(
          (element) => element!.month == widget.selectedDate!.month && element.year == widget.selectedDate!.year);
  ...
  Widget daysOfWeek(double width, String? locale, WeekDay? weekday) {
    ...
    for (var day = 12; day <= 18; day++) {
      weekday == WeekDay.long
          ? daysNames.add(DateFormat.EEEE(locale.toString()).format(DateTime.parse('1970-01-' + day.toString())))
